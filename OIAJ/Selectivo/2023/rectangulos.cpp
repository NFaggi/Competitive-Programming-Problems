...
{
    ll mi=LLONG_MAX,dis,pos=1, i, j,cant,dif,W,H,punt=1,aW=1,aH=1,n=int(x.size()),l,r,comp,inpos=1;
    vector<ll>v;
    for(i=0; i<n; i++)
    {
        dis=dist(x[i],y[i]);
    ...
    while(int(v.size())>1)
    {
        ...
        for(i=0; i<n; i++)
        {
            W=abs(x[inpos-1]-x[v[i]-1])*2;
            W=max(W,1ll);
            H=abs(y[inpos-1]-y[v[i]-1])*2;
            H=max(H,1ll);
            cant=0;
            for(j=0; j<n; j++)
            {
                l=abs(x[inpos-1]-x[v[j]-1])*2;
                l=max(l,1ll);
                r=abs(y[inpos-1]-y[v[j]-1])*2;
                r=max(r,1ll);
                if(l <= W && r <= H)
                {
                    cant++;
